...
class DDRMASTester:
    ...
    def create_system(self) -> System:
        ...
        self.system.MAX_ARGUMENTS_NUMBER_PER_QUERY = 999999999999999
        agents = {}
        ...
        head_literal = self.literals[0]
        head_literal_negated = head_literal.negated()
        definer_agent = self.agents_list[0]
...
async def perform_queries():
    ...
    query_focuses_per_ans.append(tester.system.max_arguments_times)
    sizes_messages_answers = tester.system.size_messages_answers
    ...
    arg_sizes_array = np.asarray(sizes_messages_answers)
    df_describe = pd.DataFrame(arg_sizes_array)
    logger.warning("Size of messages statistics:")
    logger.warning(df_describe.describe().astype(str))

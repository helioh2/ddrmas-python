...
async def perform_queries():
    ...
    for k in range(11):
        ...
        system = load_system_from_file("test1.yml")

        p = LLiteral(
                definer=system.agents["d1"], 
                literal=Literal(True, "aa1", ["M"])
                )
        ...
        queryfocus = QueryFocus("alfa1", 
                                ...
                                kb=[
                                     Rule("r_alfa1", LLiteral(Sign.FOCUS, Literal(False, "ag1", ["M"]))),
                                     Rule("r_alfa2", LLiteral(Sign.FOCUS, Literal(True, "af1", ["M"]))),
